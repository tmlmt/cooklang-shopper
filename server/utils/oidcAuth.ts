...
/**
 * Shared handler for the generic OIDC provider callback.
 * `basePath` is the redirect path prefix (differs between the /auth and
 * /api/auth mount points), to which the provider name is appended.
 */
  ...
  const handler = defineOAuthOidcEventHandler({
    config: {
      ...
      redirectURL: config.baseUrl
        ? `${config.baseUrl.replace(/\/$/, "")}${basePath}/${name}`
        : "",
    },
