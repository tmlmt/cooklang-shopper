...
async function main() {
  ...
  await run("tar", [
    "-czf",
    archiveName,
    // Follow and hard copy target of symlinks
    "--dereference",
    // Put main build files to ./dist
    "--transform",
    "s|^\\.output|dist|",
    // Put upgrade script to ./dist as well
    "--transform",
    "s|^scripts/upgrade.sh|upgrade.sh|",
    // Put other important files to ./dist as well
    "--transform",
    "s|^config.yaml.example|dist/config.yaml.example|",
    "--transform",
  ...
  log.success("Build and commit completed successfully!");
  ...
  if (!DRY_RUN) {
    ...
    if (answer === "n" || answer === "N") {
      ...
      if (fs.existsSync(archivePath)) {
        ...
        log.info("Cleaned up release archive.");
      }
      process.exit(0);
    }
  }
